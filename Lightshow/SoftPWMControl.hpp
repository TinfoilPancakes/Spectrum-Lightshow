...
protected:
    ...
    std::chrono::duration<unsigned long long, std::nano> period = std::chrono::milliseconds(8); //Defaults to 8ms period time.
    ...
    static void Execute(SoftPWMControl* controller, std::mutex* m){
        ...
        while (shouldContinue) {
            m->lock();
            controller->pin->setPinValue(short(GPIO_PIN_ON * (controller->dutyCycle != 0))); //The ceil function is to keep full off at 0% duty.
            m->unlock();
...
public:
    ...
    SoftPWMControl(GPIOInterface* pin, const float& dutyCycle, const std::chrono::duration<unsigned long long, std::nano>& period) : SoftPWMControl(pin, dutyCycle) {
        this->period = period;
    }
