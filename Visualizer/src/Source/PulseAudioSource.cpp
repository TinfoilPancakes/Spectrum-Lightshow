...
bool vis::PulseAudioSource::open_pulseaudio_source(
  ...
  if (audio_device.empty()) {
    ...
    if (!m_pulseaudio_default_source_name.empty()) {
      m_pulseaudio_simple = pa_simple_new(
          nullptr, k_record_stream_name, PA_STREAM_RECORD,
          m_pulseaudio_default_source_name.c_str(), k_record_stream_description,
    ...
    if (m_pulseaudio_simple == nullptr) {
      m_pulseaudio_simple =
          pa_simple_new(nullptr, k_record_stream_name, PA_STREAM_RECORD, "0",
                        k_record_stream_description, &sample_spec, nullptr,
                        &buffer_attr, &error_code);
    }
  } else {
    m_pulseaudio_simple =
        pa_simple_new(nullptr, k_record_stream_name, PA_STREAM_RECORD,
