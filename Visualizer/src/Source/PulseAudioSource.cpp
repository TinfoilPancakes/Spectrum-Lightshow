...
void vis::PulseAudioSource::pulseaudio_context_state_callback(pa_context* c,
	...
	case PA_CONTEXT_READY: {
		pa_operation_unref(
		    pa_context_get_server_info(c,
		                               pulseaudio_server_info_callback,
		                               userdata));
		break;
	}
...
void vis::PulseAudioSource::populate_default_source_name() {
	...
	pa_context_connect(pulseaudio_context,
	                   nullptr,
	                   PA_CONTEXT_NOFLAGS,
	                   nullptr);
...
bool vis::PulseAudioSource::open_pulseaudio_source(
	...
	static const pa_sample_spec sample_spec = {PA_SAMPLE_S16LE,
	                                           k_sample_rate,
	                                           k_channels};

	static const pa_buffer_attr buffer_attr = {max_buffer_size,
	                                           0,
	                                           0,
	                                           0,
	                                           (max_buffer_size / 2)};
	...
	if (audio_device.empty()) {
		...
		if (!m_pulseaudio_default_source_name.empty()) {
			m_pulseaudio_simple =
			    pa_simple_new(nullptr,
			                  k_record_stream_name,
			                  PA_STREAM_RECORD,
			                  m_pulseaudio_default_source_name.c_str(),
			                  k_record_stream_description,
			                  &sample_spec,
			                  nullptr,
			                  &buffer_attr,
			                  &error_code);
		...
		if (m_pulseaudio_simple == nullptr) {
			m_pulseaudio_simple = pa_simple_new(nullptr,
			                                    k_record_stream_name,
			                                    PA_STREAM_RECORD,
			                                    nullptr,
			                                    k_record_stream_description,
...
bool vis::PulseAudioSource::read(pcm_stereo_sample* buffer,
	...
	if (m_pulseaudio_simple != nullptr) {
		...
		int32_t error_code;
		/* Record some data ... */
		auto return_code = pa_simple_read(m_pulseaudio_simple,
		                                  buffer,
		                                  buffer_size_bytes,
		                                  &error_code);
