...
vis::SpectrumTransformer::SpectrumTransformer(
  ...
  m_fftw_results = (static_cast<size_t>(m_settings->get_sample_size()) / 2) + 1;
...
void vis::SpectrumTransformer::execute(pcm_stereo_sample *buffer,
  ...
  auto right_margin =
      static_cast<int32_t>(m_settings->get_spectrum_right_margin() * win_width);
  auto left_margin =
      static_cast<int32_t>(m_settings->get_spectrum_left_margin() * win_width);
  ...
  if (m_silent_runs < k_max_silent_runs_before_sleep) {
    ...
    if (m_shared_ext_data.get_data_ptr()) {
      ...
      memcpy(m_shared_ext_data.get_data_ptr(), &external_frame,
             sizeof(external_frame));
      memcpy(m_shared_ext_data.get_data_ptr() + sizeof(external_frame),
             m_fftw_output_left, sizeof(fftw_complex) * m_fftw_results);
      if (is_stereo)
        memcpy(m_shared_ext_data.get_data_ptr() + sizeof(external_frame) +
                   sizeof(fftw_complex) * m_fftw_results,
               m_fftw_output_right, sizeof(fftw_complex) * m_fftw_results);
...
void vis::SpectrumTransformer::apply_falloff(
  ...
  for (auto i = 0u; i < bars.size(); ++i) {
    ...
    auto falloff_value =
        std::min((*falloff_bars)[i] * m_settings->get_spectrum_falloff_weight(),
                 (*falloff_bars)[i] - 1);
...
void vis::SpectrumTransformer::scale_bars(const int32_t height,
  ...
  max_height = std::max(max_height, 1.0); // avoid division by zero when height
...
void vis::SpectrumTransformer::draw_bars(
    ...
    vis::NcursesWriter *writer) {
  recalculate_colors(static_cast<size_t>(win_height), m_settings->get_colors(),
  ...
  auto top_margin = static_cast<int32_t>(m_settings->get_spectrum_top_margin() *
                                         full_win_height);
  ...
  for (auto original_column_index = 0u; original_column_index < bars.size();
    ...
    if (m_settings->get_spectrum_falloff_mode() == vis::FalloffMode::Top) {
      ...
      if (row_index > 0) {
        ...
        writer->write(top_row_height + top_margin - bottom_margin,
                      column + left_margin,
                      m_precomputed_colors[static_cast<size_t>(row_index)],
                      bar_row_msg, m_settings->get_spectrum_character());
      }
...
void vis::SpectrumTransformer::recalculate_cutoff_frequencies(
  ...
  auto freqconst =
      std::log10(static_cast<double>(m_settings->get_low_cutoff_frequency()) /
