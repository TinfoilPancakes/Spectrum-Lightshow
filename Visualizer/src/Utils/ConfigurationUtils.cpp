	...
	while (file.good() && std::getline(file, line)) {
		if (!line.empty() && line[0] != L'#') {
			...
			if (!split_line.first.empty()) {
				...
				VIS_LOG(vis::LogLevel::WARN,
				        "Configuration line was not valid at %s",
				        line.c_str());
			}
		}
	}
...
vis::ConfigurationUtils::read_colors(const std::string& colors_path) {
	...
	if (!file.good()) {
		VIS_LOG(vis::LogLevel::WARN,
		        "Colors configuration not found at %s",
		        colors_path.c_str());
	...
	for (const auto& color_line : lines) {
		if (color_line.size() >= 7) {
			...
			const auto color =
			    vis::ColorDefinition{NcursesUtils::to_ansi_color(red,
			                                                     green,
			                                                     blue),
			                         red,
			                         green,
			...
			if (basic_color.get_color_index() >= 0) {
				if (is_gradient_enabled) {
					add_color_gradients(basic_color,
					                    gradient_interval,
					                    &colors);
				} else {
...
vis::FalloffMode vis::ConfigurationUtils::read_falloff_mode(
	...
	auto falloff_mode_str = vis::Utils::get(properties,
	                                        k_spectrum_falloff_mode_setting,
	                                        std::string{""});

	vis::FalloffMode falloff_mode = default_falloff_mode;

	if (!falloff_mode_str.empty()) {
		std::transform(falloff_mode_str.begin(),
		               falloff_mode_str.end(),
		               falloff_mode_str.begin(),
		               ::tolower);
		...
		if (falloff_mode_str == "none") {
			...
			VIS_LOG(vis::LogLevel::ERROR,
			        "Invalid falloff mode %s for %s",
			        falloff_mode_str.c_str(),
			        config_param.c_str());
		}
	}
...
vis::SmoothingMode vis::ConfigurationUtils::read_smoothing_mode(
	...
	auto smoothing_mode_str = vis::Utils::get(properties,
	                                          k_spectrum_smoothing_mode_setting,
	                                          std::string{""});

	vis::SmoothingMode smoothing_mode = default_smoothing_mode;

	if (!smoothing_mode_str.empty()) {
		std::transform(smoothing_mode_str.begin(),
		               smoothing_mode_str.end(),
		               smoothing_mode_str.begin(),
		               ::tolower);
		...
		if (smoothing_mode_str == "none") {
			...
			VIS_LOG(vis::LogLevel::ERROR,
			        "Invalid spectrum mode %s for %s",
			        smoothing_mode_str.c_str(),
			        config_param.c_str());
		}
	}
...
// enable pulse audio by default if available
	...
	validate_setting_is_greater_than_zero(settings
	                                          ->get_spectrum_falloff_weight(),
	                                      k_spectrum_falloff_weight_setting);
	...
	settings->set_lorenz_character(Utils::get(properties,
	                                          k_lorenz_character_setting,
	                                          default_lorenz_char));

	settings->set_ellipse_character(Utils::get(properties,
	                                           k_ellipse_character_setting,
	                                           default_ellipse_char));
	...
	settings->set_pulse_audio_source(
	    Utils::get(properties,
	               k_vis_pulse_audio_source_setting,
	...
	const auto visualizers =
	    Utils::split(Utils::get(properties,
	                            k_visualizers_setting,
